...
void	max_heap::insert(element data)
{
	...
	{
		if (abs(tree[i]) < abs(tree[i / 2]) || (abs(tree[i]) == abs(tree[i / 2]) && tree[i] < tree[i / 2]))
...
element	max_heap::pop()
{
	...
	for (unsigned int i = 1; i * 2 <= last; )
	{
		...
		{
			if (abs(tree[i * 2]) < abs(tree[i * 2 + 1]) || (abs(tree[i *2]) == abs(tree[i * 2 + 1]) && tree[i*2] < tree[i * 2 + 1]))
				larger = i * 2;
			else
				larger = i * 2 + 1;
		}
		if (abs(tree[larger]) < abs(tree[i]) || (abs(tree[larger]) == abs(tree[i]) && tree[larger] < tree[i]))
